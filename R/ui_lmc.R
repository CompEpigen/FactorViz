lmc_tab <- function() {
  tabPanel(
    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        checkboxInput("cgVarSubset", "Select top-variable CpGs:", value =
                        FALSE)
      ),
      conditionalPanel(
        '(input.componentPlotType === "heatmap" ||
  ...
  conditionalPanel(
    ...
    selectInput(
      "mdsDist",
      "Distance:",
      c("euclidean", "angular", "correlation"),
      selected = 1
    )
  ),
  conditionalPanel(
  ...
  conditionalPanel(
    ...
    selectInput(
      "locusType",
      "Locus:",
      c("from gene sets" = "predefined", "arbitrary" = "arbitrary")
    )
  ),
  conditionalPanel(
    'input.componentPlotType === "locus plot" &&
  ...
  conditionalPanel(
    ...
    input.componentPlotType === "dendrogram" ||
    input.componentPlotType === "similarity graph" ',
    checkboxInput("correlationCentered", "Center matrices", value =
                    FALSE)
  ),
